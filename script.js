...
(() => {
  ...
  /* ---------- upload project + edit file (client-side only) ----------
     ...
     sampai kamu menekan tombol Deploy / Push ke GitHub. Untuk .zip,
     isinya dibaca & diekstrak langsung di browser memakai JSZip. File
     bertipe teks (html/css/js/json/dll) bisa diklik ✎ untuk membuka
     editor full-screen. Nama/path tiap file bisa diganti langsung di
     kolom teksnya. */
  ...
  function readFileAsBase64(file){
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = () => resolve(reader.result.split(",").pop()); // buang prefix data:...;base64,
      reader.onerror = reject;
  ...
  /* ---------- tambah 1 file ke daftar (append, bukan replace) ---------- */
  ...
  /* ---------- push langsung ke GitHub (real, lewat /api/github) ---------- */
  if (uploadGithubBtn){
